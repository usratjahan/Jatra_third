...
const Events = () => {
  ...
  const applyFilters = async () => {
    ...
    try {
      ...
      //   community, date, time, travelers,
      //   cities: selectedCities.join(','),
      //   minPrice: priceRange[0], maxPrice: priceRange[1],
      // });
      // const res = await fetch(`/api/events?${params}`);
      // const data = await res.json(); setFiltered(data);
      await new Promise((r) => setTimeout(r, 400));
      let result = [...events];
      if (selectedDivisions.length > 0)
        result = result.filter((e) => selectedDivisions.includes(e.division));
      if (community) result = result.filter((e) => e.community === community);
      if (selectedCities.length > 0)
        result = result.filter((e) =>
          selectedCities.some((c) =>
            e.location.toLowerCase().includes(c.toLowerCase()),
          ),
        );
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {/* TODO: fetch from GET /api/communities */}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
              ...
              <div className="flex flex-wrap gap-2">
                {community && (
                  <span className="flex items-center gap-1.5 px-3 py-1 bg-teal-100 text-teal-700 text-xs font-medium rounded-full">
                    👥 {community}
                    <button
